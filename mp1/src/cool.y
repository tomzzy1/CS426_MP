...
%right IN // IN should have lowest precedence to avoid conflict
%right ASSIGN
%left NOT
%nonassoc LE '<' '='
%left '+' '-'
...
class_list
        ...
        | class_list error ';' { /*printf("class list error\n");*/ }
        ;
...
feature_list:        /* empty */
        ...
        | feature_list error ';' { /*printf("feature list error\n");*/ }
        ;
...
expr_blocks: expr ';'
        ...
        | expr_blocks error ';' { /*printf("expression block error1\n");*/ }
        | error ';' { /*printf("expression block error2\n");*/ }
        ;

/* let id1:type1, id2:type2 in expr <=> let id1:type1 in let id2:type2 in expr*/
let_expr: LET OBJECTID ':' TYPEID ASSIGN expr ',' let_expr_tail
                { $$ = let($2, $4, $6, $8); }
        | LET OBJECTID ':' TYPEID ',' let_expr_tail 
                { $$ = let($2, $4 , no_expr(), $6); }
        | LET OBJECTID ':' TYPEID ASSIGN expr IN expr
                { $$ = let($2, $4, $6, $8); }
        | LET OBJECTID ':' TYPEID IN expr
                { $$ = let($2, $4 , no_expr(), $6); }
        | LET error let_expr_tail { /*printf("let error1\n");*/ }
        | LET error { /*printf("let error2\n");*/ }
        ;

let_expr_tail: OBJECTID ':' TYPEID ASSIGN expr ',' let_expr_tail
                { $$ = let($1, $3, $5, $7); }
        ...
        | OBJECTID ':' TYPEID IN expr
                { $$ = let($1, $3 , no_expr(), $5); }
        | error let_expr_tail { /*printf("let tail error\n");*/ }
        ;
...
expr: OBJECTID ASSIGN expr
        ...
        | expr '.' OBJECTID '(' expr_list ')' 
                { $$ = dispatch($1, $3, $5); }
        | OBJECTID '(' expr_list ')' // shorthand for dispatch
