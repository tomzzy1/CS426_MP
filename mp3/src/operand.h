...
public:
  ...
  op_type(const op_type& other) = default;
...
public:
  ...
  op_func_type(const op_func_type& other) = default;
  op_func_type() = default;
  bool is_ptr() { return false; }
  op_type get_ptr_type();
  op_type get_deref_type();
  bool is_pptr() { return false; }
  op_func_type replace_self_type(const string& class_name)
  {
    op_type new_res;
    vector<op_type> new_args;
    auto replace = [&](op_type o)
    {
      if (o.is_self_type())
      {
        o.set_type(op_type(class_name));
      }
      else if (o.is_same_with(op_type("SELF_TYPE", 1)))
      {
        o.set_type(op_type(class_name, 1));
        //cout << o.get_name() << '\n';
      }
      else if (o.is_same_with(op_type("SELF_TYPE", 2)))
      {
        o.set_type(op_type(class_name, 2));
      }
      return o;
    };
    new_res = replace(res);
    for (auto& o: args)
    {
      new_args.push_back(replace(o));
    }
    //std::cout << get_name() << '\n';
    return op_func_type(new_res, new_args);
  }
  op_type get_return_type()
  {
    return res;
  }
  vector<op_type>& get_args_type()
  {
    return args;
  }
};
...
op_type to_object(const op_type& o);