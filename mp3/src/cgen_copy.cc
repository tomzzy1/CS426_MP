...
void CgenNode::code_class() {
  ...
  operand object_ptr = vp.bitcast(memory, self_type);
  operand malloc_null = vp.icmp(EQ, object_ptr, null_value(op_type()));
  ...
  operand vtblpptr = vp.getelementptr(self_type.get_deref_type(), vector<operand>{
    object_ptr, zero_i32, zero_i32,
  }, vtblptr_type.get_ptr_type());
  vp.store(vtbl_prototype, vtblpptr);
  vp.store(object_ptr, alloca_op);
  size_t i = 1;
  for (; i < env.get_class()->attrs.size(); ++i)
  {
    op_type attr_type = env.get_class()->attrs[i];
    op_type object_ptr_type(object_ptr.get_type());
    operand attr_ptr = vp.getelementptr(object_ptr_type.get_deref_type(),
      object_ptr, int_value(0), int_value(i)
    ...
    operand res = env.attr_inits[i - 1]->code(&env);
    std::cerr << res.get_typename();
    if (!res.is_empty())
    {
      if (res.get_type().is_int_object())
      {
        operand res_ptr = vp.getelementptr(op_type("Int"),
          res, int_value(0), int_value(1)
          , op_type(INT32_PTR));
        operand val = vp.load(res_ptr.get_type().get_deref_type(), res_ptr);
...
void method_class::code(CgenEnvironment *env) {
  ...
  vp.end_define();
#else
  op_type ret_type = unbox(op_type(return_type->get_string()));
  vector<operand> args;
  string class_name(env->get_class()->get_type_name());
  op_type self_type(class_name, 1);
  //Entry self("self", 4, -1);
  ...
  for (int i = formals->first(); formals->more(i); i = formals->next(i))
  {
    ...
    args.push_back(operand(unbox(op_type(f->get_type_decl()->get_string())),
  ...
  //env->add_local(&self, alloca_op);
  env->self = alloca_op;
  //cerr << env->self.get_type().get_deref_type().get_name() << endl;
  for (int i = formals->first(); formals->more(i); i = formals->next(i))
  {
    Formal f = formals->nth(i);
    alloca_op = vp.alloca_mem(unbox(op_type(f->get_type_decl()->get_string())));
    alloca_ops.push_back(alloca_op);
    env->add_local(f->get_name(), alloca_op);
  }
  expr->make_alloca(env);
  ...
  if (res.get_typename() != ret_type.get_name())
  {
    ...
    operand new_res = conform(res, ret_type, env);
    vp.ret(new_res);
  }
  else
    vp.ret(res);
...
operand assign_class::code(CgenEnvironment *env) {
  ...
  if (res_type.is_int_object())
  {
    auto val_ptr = vp.getelementptr(op_type("Int"), res, 
      int_value(0), int_value(1), op_type(INT32_PTR));
    auto val = vp.load(op_type(INT32), val_ptr);
    res = val;
  }
  if (res_type.is_bool_object())
  {
    auto val_ptr = vp.getelementptr(op_type("Bool"), res, 
      int_value(0), int_value(1), op_type(INT1_PTR));
  ...
  if (!op)
  {
    ...
    res = box(res, attr.get_type().get_deref_type(), env);
    vp.store(res, attr);
    return res;
  }
  std::cerr << op->get_typename() << endl;
  res = box(res, op->get_type().get_deref_type(), env);
  //else
    vp.store(res, *op);
  return res;
}
...
operand let_class::code(CgenEnvironment *env) {
  ...
  ValuePrinter vp(*env->cur_stream);
  op_type o_type = unbox(op.get_type());
...
operand object_class::code(CgenEnvironment *env) {
  ...
  if (!op)
  {
    operand attr = env->get_attr(name);
    return attr;
    //return vp.load(attr.get_type().get_deref_type(), attr);
  }
  //return env->get_attr(name);
...
operand static_dispatch_class::code(CgenEnvironment *env) {
  ...
  ValuePrinter vp(*env->cur_stream);
  //operand cmp = vp.icmp(EQ, caller, operand(op_type(), "null"));
  //string ok = env->new_ok_label();
  ...
  operand func_ptr = vp.getelementptr(vtblptr_type, vector<operand>{
    vtbl_prototype, zero_i32, int_value(index + 4)
  }, class_nd->vtable2[index].get_ptr_type());
  op_func_type func_type = class_nd->vtable2[index];
...
operand dispatch_class::code(CgenEnvironment *env) {
  ...
  assert(0 && "Unsupported case for phase 1");
    ...
    // ADD CODE HERE AND REPLACE "return operand()" WITH SOMETHING
    // MORE MEANINGFUL
  std::cerr << "dynamic dispatch" << endl;
  ValuePrinter vp(*env->cur_stream);
  vector<operand> args;
  for (int i = actual->first(); actual->more(i); i = actual->next(i))
  {
    operand arg_result = actual->nth(i)->code(env);
    string arg_result_type = arg_result.get_typename();
    std::cerr << arg_result_type << endl;
    //if (arg_result_type == "i32" || arg_result_type == "i1" || arg_result_type == "")
    //if (!arg_result.get_type().is_pptr())
      args.push_back(arg_result);
    //else
      //args.push_back(vp.load(arg_result.get_type().get_deref_type(), arg_result));
  }
  operand caller(expr->code(env));
...
void method_class::layout_feature(CgenNode *cls) {
  ...
  for (int i = formals->first(); formals->more(i); i = formals->next(i))
  {
    ...
    arg_types.emplace_back(unbox(arg_type));
  }
  cls->methods.push_back(
    op_func_type(unbox(string(return_type->get_string())), 
    arg_types));
  string class_name = cls->get_type_name();
  //if (class_name == "Object" || class_name == "Int" || class_name == "Bool" 
  //|| class_name == "String" || class_name == "IO")
  if (cls->basic())
    cls->method_names.push_back(class_name + '_' + string(name->get_string()));
  else
    cls->method_names.push_back(class_name + '.' + string(name->get_string()));
  if (string(name->get_string()) == "main")
  {
    main_return_type = unbox(string(return_type->get_string()));
  }
#endif
}
...
void attr_class::layout_feature(CgenNode *cls) {
  ...
  {
    cls->attrs.push_back(unbox(op_type(type_name)));
  }
...
void let_class::make_alloca(CgenEnvironment *env) {
  ...
  if (type_decl->equal_string("Int", 3))
    ...
    alloca_op = vp.alloca_mem(unbox(op_type(type_decl->get_string())));
