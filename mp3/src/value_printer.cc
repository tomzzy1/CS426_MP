...
operand ValuePrinter::call(op_func_type func_type, operand func, vector<operand> args)
{
  ostream& o = *stream;
  auto result_type = func_type.get_return_type();
  auto result_op = make_fresh_operand(result_type);
  auto arg_types = func_type.get_args_type();
  check_ostream(o);
  o << "\t";
  if (result_op.get_type().get_id() != VOID)
    o << result_op.get_name() << " = ";
