...
public:
  string new_str_n()
  {
    string new_str = "." + std::to_string(str_count);
    ++str_count;
    return new_str;
  }
};

std::unordered_map<string, operand> global_string_table;
//
...
private:
  ...
  // ADD CODE HERE
public:
  vector<op_func_type> methods;
  vector<op_type> attrs;
  vector<Expression> attr_inits;
  vector<string> attr_names;
  vector<string> method_names;
  vector<op_func_type> methods_replaced;
  std::set<int> override_index;
  //vector<op_func_type> vtable2;
  //std::unordered_map<string, op_func_type> methods;
  //std::unordered_map<string, op_func_type> methods_replaced;
  std::unordered_map<string, op_func_type> vtable_final;
public:
  // COMPLETE FUNCTIONS
...
public:
  ...
  CgenNode *typename_to_class(string s)
  {
    Symbol t = idtable.lookup_string(s.data());
    return type_to_class(t);
  }
  // ADD CODE HERE
  void print()
  {
  ...
  operand get_attr(Symbol name)
  {
    ValuePrinter vp(*cur_stream);
    size_t i = 0;
    for (; i < cur_class->attrs.size(); ++i)
    {
      if (cur_class->attr_names[i] == string(name->get_string()))
      {
        //std::cerr << cur_class->attrs[i].get_name() << endl;
        break;
      }
    }
    op_type object_ptr_type(self.get_type().get_deref_type());
    std::cerr << object_ptr_type.get_name() << endl;
    operand object_ptr = vp.load(object_ptr_type, self);
    operand attr_ptr = vp.getelementptr(object_ptr_type.get_deref_type(),
      object_ptr, int_value(0), int_value(i)
      , cur_class->attrs[i].get_ptr_type());
    //std::cerr << attr_inits.size() << ' ' << i << endl;
    /*if (!attr_inits[i - 1]->no_code())
    {
      auto op = attr_inits[i - 1]->code(this);
      auto attr = vp.load(attr_ptr.get_type().get_deref_type(), attr_ptr);
      std::cerr << op.get_typename() << endl;
      if (op.get_type().is_int_object())
        vp.call({op_type{"Int", 1}, op_type(INT32)}, op_type(VOID), "Int_init",
         true, {op, attr});
      if (op.get_type().is_bool_object())
        vp.call({op_type{"Bool", 1}, op_type(INT1)}, op_type(VOID), "Bool_init",
         true, {op, attr});
      return op;
